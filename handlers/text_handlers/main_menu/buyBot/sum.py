...
async def sum(message: types.Message, state: FSMContext) -> Coroutine:
    async with state.proxy() as data:
        message_text = f"Платформа - <i>{data['platform']}</i>\nСроки - <i>{data['deadline']}</i>\nБюджет - <b>{message.text}</b>\n\nТехническое задание - <i>{data['task']}</i>"
        await message.answer(message_text)
        await bot.send_message(
            admin_chat,
            f"<b><a href='{message.from_user.url}'>{message.from_user.full_name}</a></b>, хочет купить бота. \n\n"
            + message_text,
        )
...
def register_handlers_buy_bot_sum(dp: Dispatcher) -> None:
    dp.register_message_handler(sum, state=BuyBot.sum)
